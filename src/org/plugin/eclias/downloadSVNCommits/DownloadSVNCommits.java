...
public class DownloadSVNCommits {
	...
	DownloadSVNCommits(String url, String startRevision, String endRevision, String outputFolder, String username,
			String password) {
		this.url = url;
	...
	void initializeRepository() throws Exception {
		...
		if (username != null) {
			ISVNAuthenticationManager authenticationManager = SVNWCUtil.createDefaultAuthenticationManager(username,
					password);
...
//			}
			...
			if (fileType == SVNLogEntryPath.TYPE_ADDED) {
				...
				// - trunk_FileBRenamed.java.rPrevious //previous file (if exists) which is
				// actually the file /branches/FileB.java from revision36

				System.out.println("File save to currentVersion "
						+ inputOutput.getFileNameCurrentVersion(fileNameOnRepository, revision));
				saveFileFromRepository(fileNameOnRepository, revision,
						inputOutput.getFileNameCurrentVersion(fileNameOnRepository, revision));
				...
				if (fileNameOnRepositoryFrom != null) {
					...
					saveFileFromRepository(fileNameOnRepositoryFrom, revisionFrom,
							inputOutput.getFileNamePreviousVersion(fileNameOnRepository, revision));
				}
			...
			if (fileType == SVNLogEntryPath.TYPE_MODIFIED) {
				...
				saveFileFromRepository(fileNameOnRepository, revision,
						inputOutput.getFileNameCurrentVersion(fileNameOnRepository, revision));
				saveFileFromRepository(fileNameOnRepository, revision - 1,
						inputOutput.getFileNamePreviousVersion(fileNameOnRepository, revision));
...
//				if there is a text changed between previous version "from" and current version, then we should download the previous one as well. Otherwise, we don't need to
	...
	private void saveFileFromRepository(String fileNameOnRepository, long revision, String fileNameOnDisk)
			throws Exception {
